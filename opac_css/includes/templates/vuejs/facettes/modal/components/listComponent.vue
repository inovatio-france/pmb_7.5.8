	...
	<div class="modal-content-list">
		...
		<div class="modal-list">
			<div class="modal-item" v-for="(facette, index) in facettes" :key="index">
				<input type="checkbox" v-model="selectedFacettes"
					:value="facette.facette_value" @change="check"
					class="modal-item-checkbox" /> <a @click="startFacette(facette)"
					v-html="`${facette.facette_libelle} (${facette.facette_number})`"></a>
			</div>
		</div>
	</div>
</template>
