...
// +-------------------------------------------------+
// $Id: websubscribe.inc.php,v 1.28.4.7 2024/03/18 13:43:15 rtigero Exp $
...
function verif_validite_compte() {
    ...
    if ($id_empr) {
        ...
        if ($opac_websubscribe_num_carte_auto_array[0] == "2") {
            ...
            $rqt = "SELECT CAST(SUBSTRING(empr_cb," . ($long_prefixe + 1) . ") AS UNSIGNED) AS max_cb, SUBSTRING(empr_cb,1," . ($long_prefixe * 1) .
                ") AS prefixdb FROM empr ORDER BY max_cb DESC limit 0,1"; // modif f cerovetti pour sortir dernier code barre tri par ASCII
            $res = pmb_mysql_query($rqt);
