...
// +-------------------------------------------------+
// $Id: cms_module_common_datasource_list.class.php,v 1.7.2.1 2024/09/04 10:02:17 rtigero Exp $
	...
	protected function gen_select_sort_by(){
		...
		foreach ($criterias as $criteria) {
			...
			$format_criteria = str_replace(array('(', ')'), '', $criteria);
							$select .= "<option value='".$criteria."' ".($this->parameters['sort_by'] == $criteria ? "selected='selected'" : "").">".$this->format_text($this->msg['cms_module_common_datasource_list_sort_by_'.$format_criteria])."</option>"; 
		}
		$select .= "</select>";
		return $select;
	}
