...
// +-------------------------------------------------+
// $Id: AnimationModel.php,v 1.117.2.16 2024/08/30 13:10:54 gneveu Exp $
...
class AnimationModel extends Model
{
    ...
    public static function insertMailingTypeList(array $mailingType, int $idAnimation = 0)
    {
        ...
        if ($animationModel->mailing) {
            foreach ($animationModel->mailing as $mailing) {
                if (in_array($mailing->numMailingType, $mailingType, true)) {
        ...
        foreach ($mailingType as $mailingTypeId) {
            if (!empty($mailingTypeId) && $mailingTypeId != 0 && !in_array($mailingTypeId, $already_create, true) && MailingTypeOrm::exist($mailingTypeId)) {
    ...
    private static function cleanImgCache(int $id)
    {
        ...
        foreach ($resources as $resource) {
            if (in_array(basename($resource), ["CVS", ".", ".."], true)) {
                continue;
            }
