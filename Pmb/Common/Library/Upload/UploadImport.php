...
// +-------------------------------------------------+
// $Id: UploadImport.php,v 1.1.4.2 2023/11/30 11:07:23 qvarin Exp $
...
class UploadImport
{
    ...
    {
        foreach (static::VALID_STRUCTURE as $key) {
            if (! isset($file[$key])) {
                return false;
            }
        }

        return count(static::VALID_STRUCTURE) === count($file);
    }
    ...
    public function copy(string $to, string $prefixFilename = "")
    {
        ...
        if (! is_dir($to) || ! is_writable($to)) {
            throw new \InvalidArgumentException("Directory {$to} is doesn't exist or not writable");
        }
