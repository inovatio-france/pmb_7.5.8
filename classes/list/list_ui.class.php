...
// +-------------------------------------------------+
// $Id: list_ui.class.php,v 1.234.2.30 2024/07/30 06:09:03 dgoron Exp $
...
// 		$this->set_settings_in_session();
	...
	protected function get_search_filter_simple_text($name='', $size=30) {
		...
		$size = intval($size);
		$display = "<input type='text' class='saisie-".$size."em list_ui_simple_text' name='".$this->objects_type."_".$name."' value=\"".htmlentities($this->filters[$name], ENT_QUOTES, $charset)."\" />";
	...
	protected function get_search_filter_boolean_selection($name='', $message_all='') {
		...
		if($message_all) {
			$selector .= "<span class='list_ui_search_checkbox'>";
			$selector .= "<input type='radio' id='".$this->objects_type."_".$name."_all' name='".$this->objects_type."_".$name."' value='' ".(($this->filters[$name] == '') || ($this->filters[$name] == '-1') ? "checked='checked'" : "")."/> ";
		...
		foreach ($options as $option) {
			$selector .= "<span class='list_ui_search_checkbox'>";
			$selector .= "<input type='radio' id='".$this->objects_type."_".$name."_".$option['value']."' name='".$this->objects_type."_".$name."' value='".$option['value']."' ".(($this->filters[$name] != '') && ($option['value'] == $this->filters[$name]) ? "checked='checked'" : "")."/> ";
