...
// +-------------------------------------------------+
// $Id: mysql_requirements.inc.php,v 1.1.2.1 2023/04/07 13:53:58 dbellamy Exp $
...
if(isset($db_user)){
    ...
    $mysql_table = "
        <table>
            <tr>
                ...
                <td>{$install_msg['req_mysql_suggested_table_th_4']}</td>
            </tr>
        ";
    ...
    foreach($mysql_check as $value)
    {
        ...
        <tr>
            <th>{$value['name']}</th>
            <th>{$value['suggestedValue']}</th>
            <th>{$value['value']} $state</th>
            <th>$comment</th>
        </tr>";
    }
    $mysql_table .= "</table>";
